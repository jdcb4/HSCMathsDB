...
Promote only reviewed records into `src/data/hsc-math-advanced-explanations.json`.
...
pnpm run data:generate-explanations -- --model google/gemini-3.1-flash-lite --limit 20
```
...
OpenRouter uses an OpenAI-compatible chat-completions style API, so the script can call a selected model by name. Start with the two candidate models the user named:
